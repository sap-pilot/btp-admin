...
interface HpSpace { spaceId: string; spaceName: string; destInstanceId?: string; }
interface HpLaunchpadInstance { alias: string; name: string; }
interface HpServiceData {
...
interface HpDirectory { name: string; short: string; region?: string; subaccounts: HpSubaccount[]; }
interface HpGlobalAccount {
  id?: string; name: string; cockpitRegion?: string;
  directories: HpDirectory[];
  [key: string]: unknown;
}
interface HpTemplateChild {
  name: string; url?: string; children?: HpTemplateChild[]; repeatOn?: string;
}
interface HpTemplate { name: string; fullName?: string; url?: string; children?: HpTemplateChild[]; }
interface HpTab { title: string; dirs: string[]; }
interface HomepageData {
  tabs: HpTab[];
  btp: { globalAccounts: HpGlobalAccount[]; title?: string; };
  resources: { title: string; url: string; target?: string; }[];
  templates: Record<string, HpTemplate>;
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

function buildCtx(ga: HpGlobalAccount, dir: HpDirectory, sa: HpSubaccount): Record<string, string> {
...
function findDir(short: string, gas: HpGlobalAccount[]): { dir: HpDirectory; ga: HpGlobalAccount } | null {
  for (const ga of gas) {
...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/homepage')
      .then(r => r.json() as Promise<HomepageData | null>)
      .then(d => {
        setHomepage(d);
        if (d?.tabs.length && !initialized.current) {
          setActiveTab(d.tabs[0].title);
              ...
              {homepage.tabs.map(tab => (
                <TabsContent key={tab.title} value={tab.title} className="mt-0 p-4">
                  {tab.dirs.map(dirShort => {
                    const found = findDir(dirShort, homepage.btp.globalAccounts);
                    if (!found) return null;
                    return (
                      <DirTable key={dirShort} ga={found.ga} dir={found.dir} templates={homepage.templates} />
                    );
                  })}
                </TabsContent>
              ))}
