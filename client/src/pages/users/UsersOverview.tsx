...
type UserData = Record<string, { users: UserSummary[]; total: number }>;
...
interface ModalState {
  ...
  initialTab?:          'detail' | 'access' | 'history';
}
...
function fmtLogon(ts: number | undefined): string {
  if (!ts) return '';
...
export default function UsersOverview() {
  ...
  const {
    ...
    email: emailParam,
    userTab,
  } = useParams<{ tab?: string; region?: string; subdomain?: string; origin?: string; email?: string; userTab?: string }>();
  const navigate  = useNavigate();
  const location  = useLocation();
  const returnUrl = useRef<string>('/users');
  ...
  const allUserSas = saData.filter(sa => sa.manageRoles);
  ...
  async function loadData() {
    try {
      const [tabsRes, sasRes, usersRes] = await Promise.all([
      ...
      const users = await usersRes.json() as { ok: boolean; data: UserData };
      if (tabs.ok)  setTabEntries(tabs.data);
      if (sas.ok)   setSaData(sas.data);
      if (users.ok) setUserData(users.data);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function fetchGlobalChangelog(archiveFile?: string) {
    ...
    try {
      const url = archiveFile
        ? `/api/users/global-changelog?file=${encodeURIComponent(archiveFile)}`
        : '/api/users/global-changelog';
  ...
  useEffect(() => {
    const es = new EventSource('/api/events?users=1');
      ...
      try {
        ...
        if (data['type'] === 'progress' || data['type'] === 'done') {
          ...
          void fetch('/api/users/')
            .then(r => r.json() as Promise<{ ok: boolean; data: UserData }>)
            .then(({ ok, data: d }) => { if (ok) setUserData(d); })
            .catch(() => {});
          void fetch('/api/users/global-changelog')
  ...
  useEffect(() => {
    ...
    const key = `${regionParam}/${subdomainParam}/${originParam ?? ''}/${emailParam ?? ''}/${userTab ?? ''}`;
    if (deepLinkKey.current === key) return;
    if (saData.length === 0) return;
    const sa = allUserSas.find(s => s.region === regionParam && s.subdomain === subdomainParam);
    if (!sa) return;
    deepLinkKey.current = key;
    const initialTab = userTab === 'access' ? 'access' : userTab === 'history' ? 'history' : 'detail';
    setModal({ sa, initialUserEmail: emailParam, initialUserOrigin: originParam, initialTab });
  }, [regionParam, subdomainParam, originParam, emailParam, userTab, saData]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function doRefresh(force = false) {
    ...
    setProgress(null);
    try {
      const url = force ? '/api/users/refresh?force=true' : '/api/users/refresh';
  ...
  function openModal(sa: SubaccountEntry, email?: string, origin?: string) {
    if (email && origin) {
      navigate(`/users/${sa.region}/${sa.subdomain}/${encodeURIComponent(origin)}/${encodeURIComponent(email)}`, { replace: false });
    } else {
      navigate(`/users/${sa.region}/${sa.subdomain}`, { replace: false });
    }
    setModal({ sa, initialUserEmail: email, initialUserOrigin: origin });
  }
  ...
  const visibleTabs = tabEntries.filter(te =>
    te.sections.some(s => s.type === 'subaccountGroup' && allUserSas.some(sa => csvIncludes(sa.groupIds, s.groupId))),
  );
  ...
  return (
    <div className="flex flex-col h-full bg-background text-foreground">
      ...
      <div className="border-b border-border bg-background px-3 flex items-center gap-2 shrink-0 min-h-[52px]">
        ...
        {collapsed && <Users className="h-4 w-4 sm:hidden text-muted-foreground" aria-label="Users" />}
        <div className="hidden sm:flex flex-col justify-center min-w-0">
          <span className="text-sm font-semibold leading-tight">Users</span>
      ...
      {progress && (() => {
        ...
        const pct       = isDone ? 100 : progress.total > 0 ? Math.round((Math.max((progress.current ?? 0) - 1, 0) / progress.total) * 100) : 0;
        ...
        const msg = progress.type === 'progress'
          ? `Processing ${progress.current ?? 0} of ${progress.total} subaccounts: ${progress.name ?? ''}${progress.received > 0 ? `, received ${progress.received} users` : ''}`
          : `Refreshed ${progress.refreshed ?? 0} of ${progress.total} subaccounts, received ${progress.received} users, created ${progress.created ?? 0}, updated ${progress.updated ?? 0} and deleted ${progress.deleted ?? 0}`;
        return (
      ...
      {searchActive && !isChangeHistory && (
        <div className="shrink-0 border-b border-border bg-muted/20 px-4 py-2 flex items-center gap-2 text-xs">
          <span className="text-muted-foreground">
            ...
            {' — '}{totalSearchMatches} user{totalSearchMatches !== 1 ? 's' : ''} matched
          </span>
          <button
            onClick={clearSearch}
      ...
      {/* Tab bar */}
      {(visibleTabs.length > 0 || allUserSas.length > 0) && (
        <div className="flex items-stretch border-b border-border shrink-0 px-2">
          <div className="flex items-center overflow-x-auto flex-1 min-w-0">
            {displayedTabs.map(te => (
              <button key={te.tab} className={tabCls(!isChangeHistory && te === activeTabEntry)} onClick={() => navigate(`/users/${encodeURIComponent(te.tab)}`)}>
                {te.tab}
              </button>
            ))}
          </div>
          <button
            ...
            onClick={() => navigate('/users/change-history')}
              ...
              <input
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter' && clSearch.trim()) {
                    setClSearching(true);
                    fetch(`/api/users/global-changelog/search?q=${encodeURIComponent(clSearch.trim())}`)
          ...
          <div className="flex-1 overflow-auto p-4">
            ...
            {!globalChangelogLoading && !globalChangelog && (
              <div className="text-xs text-muted-foreground text-center py-8">
                No global refresh has run yet. Click <strong>Refresh</strong> to generate the first changelog entry.
              </div>
            )}
            {!globalChangelogLoading && globalChangelog && (
        ...
        <div className="flex-1 overflow-auto px-4 py-3 space-y-6">
          {visibleTabs.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-2 text-muted-foreground">
              {isLoading
                ...
                : <>
                    <p className="text-sm">No user data configured.</p>
          ...
          {activeTabEntry?.sections
            ...
            .map(grp => {
              ...
              const maxRows   = searchActive
                ? visibleBuckets.reduce((m, b) => Math.max(m, b.users.length), 0)
                : 16;
              const rowCount  = Math.min(maxRows, 16);
                        ...
                        <tbody>
                          {Array.from({ length: rowCount }, (_, i) => (
                            <tr key={i} className="hover:bg-muted/20">
                              {visibleBuckets.map(({ sa, users }) => {
                                const user = users[i];
                                return (
                                  <Fragment key={sa.subaccountId}>
                                    <td className="px-3 py-1.5 text-xs border-b border-border border-l first:border-l-0 max-w-[180px]">
                                      {user ? (
                                        <button
                                    ...
                                    <td className="px-2 py-1.5 text-[11px] border-b border-border text-muted-foreground whitespace-nowrap w-[110px] tabular-nums">
                                      {user?.lastLogonTime ? fmtLogon(user.lastLogonTime) : ''}
                                    </td>
                                  </Fragment>
                                );
                              })}
                          ...
                          <tr className="hover:bg-muted/10">
                            {visibleBuckets.map(({ sa, total, matchCount }) => (
                              <td key={sa.subaccountId} colSpan={2} className="px-2 py-1.5 text-xs border-l first:border-l-0 border-border">
                                ...
                                >
                                  {searchActive
                                    ? `View ${matchCount} matching in modal`
                                    : `${total} users`}
                                </button>
                              </td>
                            ))}
      ...
      <AlertDialog open={showRefreshDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Refresh all users?</AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm">
                <p>Refreshing users across all subaccounts may take a while.</p>
      ...
      <AlertDialog open={showForceRefreshDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              There is an ongoing global user refresh. Would you like to force another refresh on top of it?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
