...
**Proactive refresh** — when the modal opens, the server checks whether the cached data for that subaccount is older than `DESTINATIONS_AUTO_SUBACCOUNT_REFRESH_MINS` (default 10 min; set to `0` to disable) and refreshes automatically if stale. When the Destination Overview page opens, `DESTINATION_AUTO_GLOBAL_REFRESH_HRS` (default 6 h; set to `0` to disable) governs an automatic background refresh of all subaccounts. Both vars can be set as environment variables or in `config.json → variables`; both accept fractional values (e.g. `0.5`). `DESTINATIONS_AUTO_SUBACCOUNT_REFRESH_MINS` replaces the old `DESTINATION_REFRESH_DELAY_SECONDS`.
...
4. **Subaccount Destinations modal** — opened from any destination cell, subaccount header, OTHERS button, or search result; left panel: searchable destination list with Ctrl/Shift+click multi-select for bulk export; right panel has three tabs (deep-linkable: `/destinations/:region/:subdomain/:name/history` and `/test`):
