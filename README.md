...
**Proactive refresh** — when the modal opens, the server checks whether the cached data for that subaccount is older than `DESTINATIONS_AUTO_SUBACCOUNT_REFRESH_MINS` (default 10 min) and refreshes automatically if stale. When the Destination Overview page opens, `DESTINATION_AUTO_GLOBAL_REFRESH_HRS` (default 6 h) governs an automatic background refresh of all subaccounts. Both thresholds can be set as environment variables or in `config.json → variables`; `DESTINATIONS_AUTO_SUBACCOUNT_REFRESH_MINS` accepts fractional values (e.g. `0.5` = 30 s).
...
11. **Theming** — 17 visible base themes (VS Code, Claude, Material, Art Deco, Caffeine, Corporate, Darkmatter, Ghibli Studio, Graphite, Marvel, Nature, Perpetuity, Spotify, Sunset Horizon, Twitter, and more) plus 17 accent color overlays; all colors defined in OKLCH for perceptually uniform palettes; full light/dark palette per theme applied as CSS vars with no flash on reload; selected theme persisted in localStorage; accessible via the sidebar **Themes** split-button (left half toggles light/dark, right half opens User Settings) or the **User Settings** entry in the user dropdown
...
12. **Destination Overview** — a cross-subaccount matrix view of SAP BTP Destination service configurations at `/destinations`; tabs and group IDs are read from `{LOCAL_STORE_DIR}/conf/tabs.json`; only subaccounts with `manageDestinations=true` in `subaccounts.json` appear as columns; a **Refresh** button fetches destinations from the Destination service API for each qualifying subaccount — credentials are discovered via CF v3 API by service plan guid; auth fields are resolved from `credentials` or `credentials.uaa` in the binding details and cached in `~/.ba/destination-keys.json`; tokens are resolved via a four-stage chain: (1) valid cached token, (2) `refresh_token` grant, (3) `client_credentials` with stored key (if rejected, the key is automatically re-discovered before returning an error), (4) full CF API discovery for first use — and cached in `~/.ba/destination-tokens.json`; downloads written to `{LOCAL_STORE_DIR}/dest/{region}/{subdomain}/{name}.json`; changed destinations produce a field-level diff in `{name}.changelog.md`; destinations absent from the latest API response are renamed to `{name}.deleted.json`; SSE progress bar has a dismiss X button; the grid categorises destinations as **Generic** (`API_[S4|MDG]_[HTTP|RFC]_*`), **Specific S/4** (other `API_*`), **Workzone** (`cep-*-runtime`), and **OTHERS**; clicking any destination cell, subaccount header, OTHERS button, or search result opens a **Subaccount Destinations modal**; left panel supports Ctrl/Shift+click multi-selection; **proactive load on open** — `GET /api/destinations/:region/:subdomain` checks whether `Date.now() − lastRefreshTs > DESTINATIONS_AUTO_SUBACCOUNT_REFRESH_MINS × 60000` (env var or `config.json → variables`, in minutes, default 10 min; `DESTINATION_REFRESH_DELAY_SECONDS` in seconds is still honoured as a fallback); if stale the server immediately refreshes from the Destination API, diffs, writes changelog, updates `lastRefreshTs`, and returns the fresh list; if fresh it returns the local copy; `lastRefreshTs` is in-memory (resets on server restart); **auto global refresh** — `GET /api/destinations/status` is called when the Destination Overview page opens; it checks whether `Date.now() − globalRefreshTs > DESTINATION_AUTO_GLOBAL_REFRESH_HRS × 3600000` (env var or `config.json → variables`, fractional hours, default 6 h) and triggers a background global refresh if stale; `globalRefreshTs` is set after every completed global refresh (manual or auto) and resets on server restart; the response also carries `autoGlobalRefreshHrs` and `autoSubaccountRefreshMins` so the UI can adapt; the "Updated at" timestamp below the Destination Overview title reflects the last known `globalRefreshTs`; Properties tab toolbar has **Create** (new destination from scratch), **Import** (single or array JSON), **Export** (single file or bulk `multi_destinations.json` for N selected), **Refresh** (force-refreshes from API regardless of staleness threshold, Refresh button shows Refreshing… spinner → green Refreshed ✓ → resets after 2 s; errors shown in a banner below the title bar with ✕ dismiss), **Select for Compare** (toggle — adds/removes the current destination from the comparison basket), **Reset**, and **Save Destination** above the editable table; save result shown as a banner below the toolbar — green success auto-dismisses after 3 s, error stays with ✕ close; sensitive fields (password/secret/credential) are transmitted as plaintext and masked by default with a lock icon and eye-reveal toggle; the server returns a `sensitiveFields` array alongside raw data to drive the client UI; server-side diff/changelog on every save, import, or create; Change History tab renders the raw changelog; Test Destination tab has request headers + body editor and a ~50% response panel with mock status, headers, and body (Send disabled); **Compare Destinations** — a `Compare (N) ▾` split-button in the title bar accumulates destinations across any subaccounts; the ▾ dropdown lists selected destinations (`{region} → {subdomain} → {name}`) with per-item and **Clear all** removal; clicking **Compare (N)** opens a full-screen comparison modal with all property keys as rows and one column per destination; rows with differing values are highlighted in amber; cells are editable (sensitive fields masked with eye-reveal); per-column **Save** / **Reset** buttons in the footer row — Save is enabled only when edits exist and writes back via `PUT /api/destinations/:region/:subdomain/:name` (server-side diff appended to `{name}.changelog.md`); save result banners appear under the modal title (success auto-dismisses after 3 s, error stays with ✕)
...
- **Destination Overview** (`/destinations`) — cross-subaccount matrix of SAP BTP Destination service configurations; tabs and group sections mirror the Home page layout; only subaccounts with `manageDestinations = true` appear as columns; destinations are grouped into rows: **Generic** (`API_[S4|MDG]_[HTTP|RFC]_*`), **Specific S/4** (other `API_*`), **Workzone** (`cep-*-runtime`), and **OTHERS**

- **Refresh** — **Refresh** button fetches all destination configurations from the Destination API for all managed subaccounts; changed destinations produce a field-level diff written to `{name}.changelog.md`; removed destinations renamed to `{name}.deleted.json`; SSE progress bar shows per-subaccount progress with a dismiss button

- **Full-text search** — pressing Enter on the Destination Overview search input triggers a server-side scan of all `{LOCAL_STORE_DIR}/dest/` JSON files; results filter the tab/column/row view and highlight matched text; an active-filter chip shows the query and total match count

- **Subaccount Destinations modal** — opened from any destination cell, subaccount header, OTHERS button, or search result; left panel: searchable destination list with Ctrl/Shift+click multi-select for bulk export; right panel has three tabs:
  - **Properties** — editable key/value table; sensitive fields (password, secret, credential) masked with a lock icon and eye-reveal toggle; toolbar: **Create** (new destination from scratch), **Import** (single or array JSON), **Export** (single file or `multi_destinations.json` for N selected), **Refresh** (force-refreshes this subaccount), **Select for Compare**, **Reset**, **Save**; save result shown as an inline banner (green auto-dismisses after 3 s, error stays with ✕)
...
- **Compare Destinations** — `Compare (N) ▾` split-button accumulates destinations from any subaccount across any tab; the ▾ dropdown lists selected destinations with per-item and **Clear all** removal; clicking **Compare (N)** opens a full-screen modal with all property keys as rows and one column per destination; rows where values differ are highlighted in amber; cells are editable (sensitive fields masked with eye-reveal); per-column **Save** writes back via `PUT /api/destinations/:region/:subdomain/:name` and appends a diff to the destination's changelog; save result banners appear under the modal title
